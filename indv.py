...
class INDIVIDUAL:
    ...
    def Compute_Fitness(self, env):
        ...
        # ################################### joint angles ###################################
        self.prop_data = []
        self.prop_data.append(self.sim.get_sensor_data(sensor_id=self.robot.propSensUp0))
        self.prop_data.append(self.sim.get_sensor_data(sensor_id=self.robot.propSensUp2))
        self.prop_data.append(self.sim.get_sensor_data(sensor_id=self.robot.propSensUp4))
        self.prop_data.append(self.sim.get_sensor_data(sensor_id=self.robot.propSensUp6))
        self.prop_data.append(self.sim.get_sensor_data(sensor_id=self.robot.propSensLow1))
        self.prop_data.append(self.sim.get_sensor_data(sensor_id=self.robot.propSensLow3))
        self.prop_data.append(self.sim.get_sensor_data(sensor_id=self.robot.propSensLow5))
        self.prop_data.append(self.sim.get_sensor_data(sensor_id=self.robot.propSensLow7))

        # self.avg_ang = []
        # for i in range(0, len(prop_data)):
        #     self.avg_ang.append(round(abs(np.average(self.prop_data[i])), 4))
        #     utils.graph(self.prop_data[i], True, "avg" + str(self.avg_ang[i]))
        #     print("avg ang", self.avg_ang[i])
        ...
        # self.jointAngMult = 1 / (1 + self.avgAngJ0 + self.avgAngJ1 + self.avgAngJ2 + self.avgAngJ3 + \
        ...
        self.avgCosThetaBotLight = abs(np.average(cosThetaBotLight))
        ...
        # print("JJJJJJJJJJOINTAM", self.jointAngMult)
        # print("CCCCCCCCCCCCCCCC", self.avgCosThetaBotLight)

        self.avg_ang = []
        for i in range(0, len(self.prop_data)):
            self.avg_ang.append(round(abs(np.average(self.prop_data[i])), 4))
            # utils.graph(self.prop_data[i], False, "avg" + str(self.avg_ang[i]))
            # print("avg ang", self.avg_ang[i])

        fitnessToAdd = 1000 * (self.distTarget[-1] ** -2) * self.avgCosThetaBotLight # * self.jointAngMult * speedFitnessFactor

        for i in range(0, len(self.avg_ang)):
            if self.avg_ang[i] > 0.25:
                fitnessToAdd /= 3
