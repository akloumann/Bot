...
class ROBOT:
    ...
    def send_synapses(self, sim, wts):
        ...
        for i in self.H:
            sim.send_synapse(source_neuron_id=self.H[i], target_neuron_id=self.H[i], weight=1.0)
        for i in self.MN:
            sim.send_synapse(source_neuron_id=self.MN[i], target_neuron_id=self.MN[i], weight=1.0)
